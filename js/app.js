...
// setupEvents moved to js/input/events.js
...
// drawStairs moved to js/render/drawStairs.js

// drawHandlesForStairs moved to js/render/drawStairs.js

// drawPergola moved to js/render/drawPergola.js

// drawBalcony moved to js/render/drawBalcony.js

// drawPool and drawHandlesForPool moved to js/render/drawPool.js
...
// drawHandlesForBalcony moved to js/render/drawBalcony.js

// drawGarage moved to js/render/drawGarage.js

// drawHandlesForGarage moved to js/render/drawGarage.js
...
function renderLoop() {
  ...
  // Compute UI fade alpha based on last interaction (camera or input). This controls
    ...
    for (var iR=0;iR<roofComponents.length;iR++){
      ...
      try {
        if (roof && roof.autoBase !== false && typeof computeRoofBaseHeight==='function') {
      ...
      var rCY= baseH + hgt/2;
      if(!withinCull(roof.x,rCY,roof.z)) continue;
      var rScreen=project3D(roof.x,rCY,roof.z);
      if(rScreen && !isOffscreenByCenter(rScreen)){
        var dxr=roof.x-camera.targetX, dzr=roof.z-camera.targetZ; var dr=Math.sqrt(dxr*dxr+dzr*dzr);
        allObjects.push({object:roof,type:'roof',distance:dr,maxHeight:baseH+hgt});
      }
    }
  ...
  function safeDraw(kind, obj){
    try {
      switch(kind){
        case 'room': return drawRoom(obj);
        case 'stairs': return drawStairs(obj);
        case 'furniture': return drawFurniture(obj);
        case 'balcony': return drawBalcony(obj);
        case 'pergola': return drawPergola(obj);
        case 'garage': return drawGarage(obj);
        case 'pool': return drawPool(obj);
        case 'roof': return drawRoof(obj);
      }
    } catch(e) {
      console.warn('Draw failed for', kind, e);
    }
  }
  var selectedEntry=null; for (var iO=0;iO<allObjects.length;iO++){ var it=allObjects[iO]; if(selectedRoomId && it.object && it.object.id===selectedRoomId){ selectedEntry=it; continue;} safeDraw(it.type, it.object); }
  if(selectedEntry){ safeDraw(selectedEntry.type, selectedEntry.object); }
  ...
  animationId = requestAnimationFrame(renderLoop);
  // Emit a one-time first-render event so the splash can hide precisely after the first frame
  try {
  ...
  try {
    ...
    for (var gi = 0; gi < groups.length; gi++) {
      ...
      if (tag.startsWith('room')) {
        ...
        var roof = { id: 'roof_'+Date.now()+Math.random().toString(36).slice(2), x: cx, z: cz, width: width, depth: depth, baseHeight: minY, height: height, name: 'Imported Roof', type: 'roof', roofType: 'flat', rotation: 0, autoBase:false };
        ...
        for (var ewi = 0; ewi < endpointWalls.length; ewi++) {
          ...
          if (horizontal) {
            ...
            for (var xi2 = 0; xi2 < uniq.length - 1; xi2++) {
              ...
              addHalfEdge(aK, bK, wallIdx);
              addHalfEdge(bK, aK, wallIdx);
            }
          } else {
            // vertical
            ...
            for (var yi2 = 0; yi2 < uniqY.length - 1; yi2++) {
              ...
              addHalfEdge(aK2, bK2, wallIdx);
              addHalfEdge(bK2, aK2, wallIdx);
            }
          }
        }
