...
(function(){
  ...
  if (typeof window.rebuildRoomPerimeterStrips === 'undefined') window.rebuildRoomPerimeterStrips = function(thickness){
    try {
      ...
      try { var dragId=window.__activelyDraggedRoomId||null; if(dragId){ var theRoom=null; for (var ri2=0; ri2<window.allRooms.length; ri2++){ var rr=window.allRooms[ri2]; if(rr&&rr.id===dragId){ theRoom=rr; break; } }
        if(theRoom){ var levD=(theRoom.level||0); var minX=Infinity,maxX=-Infinity,minZ=Infinity,maxZ=-Infinity; if(Array.isArray(theRoom.footprint)&&theRoom.footprint.length>0){ for(var pi=0; pi<theRoom.footprint.length; pi++){ var p=theRoom.footprint[pi]; if(!p) continue; minX=Math.min(minX,p.x); maxX=Math.max(maxX,p.x); minZ=Math.min(minZ,p.z); maxZ=Math.max(maxZ,p.z);} } else { var hw=(theRoom.width||0)/2, hd=(theRoom.depth||0)/2; minX=(theRoom.x||0)-hw; maxX=(theRoom.x||0)+hw; minZ=(theRoom.z||0)-hd; maxZ=(theRoom.z||0)+hd; }
          var pad=0.8; minX-=pad; maxX+=pad; minZ-=pad; maxZ+=pad; function inside(x,z){ return x>=minX && x<=maxX && z>=minZ && z<=maxZ; }
          window.wallStrips=(window.wallStrips||[]).filter(function(ws){ if(!ws) return false; if((ws.level||0)!==levD) return true; var inA=inside(ws.x0||0,ws.z0||0), inB=inside(ws.x1||0,ws.z1||0); return !(inA&&inB); }); }
      } } catch(_eDragPurge) {}
      ...
      function openingsForEdge(room,x0e,z0e,x1e,z1e,isRect,rectMeta){ var outs=[]; if(!room||!Array.isArray(room.openings)) return outs; var EPS=0.06; for (var oi=0; oi<room.openings.length; oi++){ var op=room.openings[oi]; if(!op) continue; if (typeof op.x0==='number'&&typeof op.z0==='number'&&typeof op.x1==='number'&&typeof op.z1==='number'){ var i0=pointSegInfo(op.x0,op.z0,x0e,z0e,x1e,z1e); var i1=pointSegInfo(op.x1,op.z1,x0e,z0e,x1e,z1e); if(i0&&i1&&i0.d<=EPS&&i1.d<=EPS&&i0.u>=-1e-3&&i1.u<=1+1e-3){ outs.push({ type:op.type, x0:op.x0, z0:op.z0, x1:op.x1, z1:op.z1, sillM:(op.sillM||0), heightM:op.heightM, meta:(op.meta||null) }); } } else if (isRect && op && typeof op.edge==='string' && typeof op.startM==='number' && typeof op.endM==='number' && rectMeta){ var edge=op.edge; var sM=op.startM, eM=op.endM; if(eM<sM){ var tmp=sM; sM=eM; eM=tmp; } var xL=rectMeta.xL,xR=rectMeta.xR,zT=rectMeta.zT,zB=rectMeta.zB; var wx0,wz0,wx1,wz1,match=false; if(edge==='minZ'){ if(Math.abs(zT - z0e)<=1e-6 && Math.abs(zT - z1e)<=1e-6){ wx0=xL+sM; wx1=xL+eM; wz0=zT; wz1=zT; match=true; } } else if(edge==='maxZ'){ if(Math.abs(zB - z0e)<=1e-6 && Math.abs(zB - z1e)<=1e-6){ wx0=xL+sM; wx1=xL+eM; wz0=zB; wz1=zB; match=true; } } else if(edge==='minX'){ if(Math.abs(xL - x0e)<=1e-6 && Math.abs(xL - x1e)<=1e-6){ wz0=zT+sM; wz1=zT+eM; wx0=xL; wx1=xL; match=true; } } else if(edge==='maxX'){ if(Math.abs(xR - x0e)<=1e-6 && Math.abs(xR - x1e)<=1e-6){ wz0=zT+sM; wz1=zT+eM; wx0=xR; wx1=xR; match=true; } } if(match){ outs.push({ type:op.type, x0:wx0, z0:wz0, x1:wx1, z1:wz1, sillM:(op.sillM||0), heightM:op.heightM, meta:(op.meta||null) }); } } } return outs; }
