...
class NoCacheHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_ai_generate(self, data):
        ...
        if provider == 'openai':
            ...
            # Replicate API - hosts many models including Flux, SDXL, etc.
            import requests as req_lib
            
            base_url = (endpoint or 'https://api.replicate.com/v1').rstrip('/')
            model_id = model or 'black-forest-labs/flux-1.1-pro'
            ...
            # Add image for image-to-image if available
            if base64_image_data:
                image_data = base64_image_data
                if not image_data.startswith('data:'):
                    image_data = 'data:image/png;base64,' + image_data
            ...
            try:
                ...
                print(f"[Replicate] Create prediction status: {resp.status_code}")
                ...
                if resp.status_code in [200, 201]:
                    ...
                    for attempt in range(max_attempts):
                        if status == 'succeeded':
                            ...
                            return {'error': pred_data.get('error', 'Prediction failed')}
                        
                        time.sleep(1)
                        poll_resp = req_lib.get(f"{base_url}/predictions/{pred_id}", headers=headers, timeout=30)
                        if poll_resp.status_code == 200:
                            pred_data = poll_resp.json()
                            status = pred_data.get('status')
                            print(f"[Replicate] Poll {attempt+1}: status={status}")
                    
                    return {'error': 'Replicate prediction timed out'}
                else:
                    return {'error': f'Replicate API error: {resp.text[:200]}'}
            except Exception as e:
            ...
            base_url = (endpoint or 'https://cloud.leonardo.ai/api/rest/v1').rstrip('/')
            model_id = model or 'phoenix'
            ...
            try:
                ...
                if base64_image_data:
                    ...
                    gen_body['init_strength'] = 0.3
                
                resp = req_lib.post(f"{base_url}/generations", json=gen_body, headers=headers, timeout=30)
                print(f"[Leonardo] Create generation status: {resp.status_code}")
                ...
                if resp.status_code == 200:
                    ...
                    if gen_id:
                        ...
                        for attempt in range(max_attempts):
                            time.sleep(2)
                            poll_resp = req_lib.get(f"{base_url}/generations/{gen_id}", headers=headers, timeout=30)
                            if poll_resp.status_code == 200:
                                poll_data = poll_resp.json()
                                status = poll_data.get('generations_by_pk', {}).get('status')
                                print(f"[Leonardo] Poll {attempt+1}: status={status}")
                                
                                if status == 'COMPLETE':
                                    images = []
                                    for img in poll_data.get('generations_by_pk', {}).get('generated_images', []):
                                        if img.get('url'):
                                            images.append(img['url'])
                                    return {'images': images, 'provider': provider}
                                elif status == 'FAILED':
                    ...
                    return {'error': f'Leonardo API error: {resp.text[:200]}'}
            except Exception as e:
                return {'error': f'Leonardo error: {str(e)}'}
        
        elif provider == 'fal':
            # fal.ai API - Fast inference
            import requests as req_lib
            
            base_url = (endpoint or 'https://fal.run').rstrip('/')
            ...
            try:
                ...
                resp = req_lib.post(url, json=input_data, headers=headers, timeout=120)
                print(f"[fal.ai] Response status: {resp.status_code}")
                
                if resp.status_code == 200:
                    resp_data = resp.json()
                    images = []
                    for img in resp_data.get('images', []):
                        if isinstance(img, dict) and img.get('url'):
                            images.append(img['url'])
                        elif isinstance(img, str):
                            images.append(img)
                    ...
                    return {'error': f'fal.ai API error: {resp.text[:200]}'}
            except Exception as e:
            ...
            base_url = (endpoint or 'https://api.together.xyz/v1').rstrip('/')
            ...
            try:
                ...
                resp = req_lib.post(url, json=body, headers=headers, timeout=120)
                print(f"[Together] Response status: {resp.status_code}")
                ...
                if resp.status_code == 200:
                    ...
                    return {'error': f'Together API error: {resp.text[:200]}'}
            except Exception as e:
                return {'error': f'Together error: {str(e)}'}
        
        elif provider == 'fireworks':
            # Fireworks AI API
            import requests as req_lib
            
            base_url = (endpoint or 'https://api.fireworks.ai/inference/v1').rstrip('/')
            model_id = model or 'accounts/fireworks/models/flux-1-dev-fp8'
            ...
            try:
                url = f"{base_url}/images/generations"
                resp = req_lib.post(url, json=body, headers=headers, timeout=120)
                print(f"[Fireworks] Response status: {resp.status_code}")
                ...
                if resp.status_code == 200:
                    ...
                    return {'error': f'Fireworks API error: {resp.text[:200]}'}
            except Exception as e:
                return {'error': f'Fireworks error: {str(e)}'}
        
        elif provider == 'ideogram':
            # Ideogram API
            import requests as req_lib
            
            base_url = (endpoint or 'https://api.ideogram.ai').rstrip('/')
            ...
            if base64_image_data:
                ...
                if not image_data.startswith('data:'):
                    image_data = 'data:image/png;base64,' + image_data
                body['image_request']['init_image'] = image_data
                body['image_request']['init_image_strength'] = 0.3
            
            try:
                url = f"{base_url}/generate"
                resp = req_lib.post(url, json=body, headers=headers, timeout=120)
                print(f"[Ideogram] Response status: {resp.status_code}")
                ...
                if resp.status_code == 200:
                    ...
                    return {'error': f'Ideogram API error: {resp.text[:200]}'}
            except Exception as e:
                return {'error': f'Ideogram error: {str(e)}'}
        
        elif provider == 'runway':
            # Runway ML API
            import requests as req_lib
            
            base_url = (endpoint or 'https://api.runwayml.com/v1').rstrip('/')
            ...
            if base64_image_data:
                ...
                if not image_data.startswith('data:'):
                    image_data = 'data:image/png;base64,' + image_data
                body['initImage'] = image_data
            
            try:
                url = f"{base_url}/image_to_image"
                resp = req_lib.post(url, json=body, headers=headers, timeout=120)
                print(f"[Runway] Response status: {resp.status_code}")
                
                if resp.status_code == 200:
                    resp_data = resp.json()
                    images = []
                    if resp_data.get('imageUrl'):
                        images.append(resp_data['imageUrl'])
                    for img in resp_data.get('images', []):
                    ...
                    return {'error': f'Runway API error: {resp.text[:200]}'}
            except Exception as e:
                return {'error': f'Runway error: {str(e)}'}
